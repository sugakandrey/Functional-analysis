...
\begin{ex}
  ...
  \end{aligned}$
      ...
      \begin{itemize}
          \item{in $\R^n$:}
              $\bar{x}_m \to \bar{x} \iff \forall j = 1, \dotsc, n\quad\ \ \:\,x_j^{(m)} \xrightarrow[m \to \infty]{} x_j$
          \item{in $\R^\infty$:}
...
\begin{defn}
  ...
  \begin{proof}\leavevmode
    \begin{itemize}
      ...
      $\phi(|x^{(m)}_k - x_k|) \leq 2^k \rho(x^{(m)}, x)$ \\
      Let $\rho(x^{(m)}, x) \le \dfrac{\epsilon}{2^k}$, then $\phi(|x^{(m)}_k - x_k|) < \epsilon$ \\
      $|x^{(m)}_k - x_k| = t = \dfrac{1}{1 - \phi(t)} - 1$, then $t \to 0$
      \item $\Leftarrow$

      Let's choose $k_0$ for which $\sum\limits_{k=k_0+1}^{\infty} \dfrac{1}{2^k} < \epsilon$ \\
      Let's choose $m_0$ for which $\forall k \leq k_0, m > m_0: |x_k^{(m)} - x_k| < \epsilon$. \\
      Then $\rho(x^{(m)}, x) < \sum\limits_{k=1}^{k_0} \dfrac{\epsilon}{2^k} + \epsilon < 2 \epsilon$ \\
...
\begin{defn}
  ...
  \R^\infty$ --- complete (by completeness of rational numbers). \\
...
\begin{thm}
  ...
  \begin{enumerate}
      ...
      \item $r_n \to 0$
    \end{enumerate}
  \underline{Then:} $\exists ! a \in \bigcap\limits_n \overline{V}_{r_n}$
\end{thm}
...
\begin{proof}\leavevmode
  \begin{itemize}
    \item Totally bounded $\implies$ compact
      ...
      $x_{n_k}$ converges in $K$?
      \[
        \epsilon_k \downarrow \to 0 \quad \epsilon_1 \quad K \subset
        \bigcup\limits_{j = 1}^p \overline{V}_j,\ rad = \epsilon_1\qquad
        \text{($\epsilon_1$-net)} 
      \]
      $p$ is finite $\implies$ one ball will contain infinetely many $x_n$
      elements. Let's name that ball $\overline{V}_{j_0}$

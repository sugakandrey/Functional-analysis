...
\begin{proof}
  ...
  X_{n_0} \cap \overline{V}_{n_0} = \varnothing \contr\ a \in \overline{V}_{n_0} \qedhere$
\end{proof}
\begin{cor}
...
\begin{proof}
  No isolated points are present $\implies$ every point in the set is nowhere dense in it. Let $X$ be countable:
    $X = \bigcup\limits_n \{X_n\}$, then it is first Baire category set. $\contr\qedhere$
\end{proof}
\begin{defn}
  $K$ --- \textbf{compact} if
  \begin{enumerate}
...
\begin{proof}
  ...
  \xRightarrow{by\ compl.} \underbrace{\bigcap\limits_{n = 1}^\infty K_n}_{\diam{K_n} \to 0,\ \{x\}} \neq \varnothing$ \\
...
\begin{ex}
  ...
  \item $L_p (E) = \Set{f - \textnormal{measurable},\ \int\limits_E |f|^p < +
    \infty}\\
...
\begin{proof}
  ...
  \textnormal{but}\ l_1 \dots l_n\ \textnormal{are linearly independent}\ \contr

...
\end{cor}
\begin{proof}
  $\forall x_1 \in S,\ Y_1 = \alpha\{x_1\}$ --- finite dimensional linear set.
  ...
  \begin{align*}
    ...
    & Y_2 = \alpha\{x_1, x_2\}\ \exists x_3 \in S\colon \|x_3 - x_j\| > \dfrac 12,\ j = 1,2
  \end{align*}
...
\]
In this case the Pythagoras theorem takes place:
\[
\norm{x+y}^2 = {\norm x}^2 + {\norm y}^2
\]

\noindent In inner product spaces the parallelogram equality plays a significant part:
\[
...
It can be proved that it's possible iff the norm complies to the parallelogram equality.
So, $C_{[a, b]}$ is not an inner product space

\begin{defn}
  \textbf{Orthonormal set} is a set of points $\Set{l_1, l_2, \dotsc}$ (may be finite):
  \begin{enumerate}
    \item $\norm{l_i} = 1$
...
\noindent This fact lets us build the orthogonal series theory effectively.
...
\begin{defn}
  $E_Y (x)$ is \textbf{best approximation} of point $x$ with points of the subspace $Y$, if $\exists y^* \in Y \quad E_y(x) = \norm{x - y^*}$ --- then $y^*$ is an element of best approximation.
\end{defn}
...
\begin{proof}
  ...
  Easy to find that there always is a ball $B(0, r) \subset \R^n$, outside which $f > 2E_Y(x)$. So, $E_Y(x)$ is somewhere inside. But $f$ is continous, the ball is compact, so, by the Weierstrass theorem, the minimum exists and is on the sphere.
\end{proof}

For abstract Fourier series the Borel theorem can be significantly reinforced by specifying the best approximation element.

\begin{thm}[extremal quality of Fourier series' partial sums]
    $\Set{e_j}$ --- ONS in $X$ \\
    $H_n = \L(l1, \dotsc, l_n)$ \\

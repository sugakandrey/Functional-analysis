...
\begin{proof}
  ...
  \text{--- subspace}) \\
  By definition of best approximation element \|x - (x_1 - \lambda^2)\| \geq \|x
  - x_1\|^2 \forall \lambda > 0 \\$
  Раскроем квадраты норм через скалярное произедение \\
  $\inprod{\underbrace{x - x_1}_{x_2} - \lambda y, x - x_1 - \lambda y} \geq
  \inprod{x - x_1, x - x_1} \\
...
\dfrac{1}{2^n}\dfrac{p_(x - y)}{1 + p_n(x - y)}\ R^\infty$
...
\begin{ex}
  ...
  p_n(x) = \max\limits_{[a, b]}|x^{(n)}(t)|\ n = 0,1,2,\ dotsc$
\end{ex}
...
\begin{proof}
  ...
  q_n(x_m - x) \ to 0$. Backwards proof is the same.
\end{proof}

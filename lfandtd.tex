...
%   \in Y$ \\
%   $[x] = \Set{y | y \sim x}$ \\
%   $\factor{X}{Y} = \Set{[x]}$ --- factor set. \\
%   $[x] + [y] = [x + y],\ \alpha [x] = [\alpha x]$ 
%   $\factor{X}{Y}$ --- linear set.
% \end{defn}

% \begin{defn}
%   $\codim{Y} = \dim{\factor{X}{Y}}$
% \end{defn}

% \begin{stm}
%   Let $\dim(\factor{X}{Y}) = p$, then $\exists e_1, \dotsc, e_p \in X$ --- lin. indep. \\
%   $\forall x \in X\ x = \sum\limits_{k = 1}^p \alpha_k e_k + y,\ y \in Y$
% \end{stm}

% If $\codim{Y} = 1$, $Y$ is called hyperplane in $X$.\\
...
%     \dfrac{x_0}{f(x_0)}$ \\
%     $f(e) = \dfrac{f(x_0)}{f(x_0)} = 1.\ \forallx \in X \exists ? t \in \R
